...
async function main() {{
  ...
  // 1. Derive encryption keys from a seed (uses the address as seed)
  const seed = new TextEncoder().encode(address);
  const encryptionKeys = await UserShareEncryptionKeys.fromRootSeedKey(seed, curve);
  ...
  // prepareDKG(protocolPublicParameters, curve, encryptionKey, bytesToHash, senderAddress)
  ...
  console.log(JSON.stringify({{
    ...
    signerPublicKey:            Array.from(encryptionKeys.getSigningPublicKeyBytes()),
    encryptionKeyAddress:       encryptionKeys.getSuiAddress(),
    encryptionKey:              Array.from(encryptionKeys.encryptionKey),
    encryptionKeySignature:     Array.from(encryptionKeys.getEncryptionKeySignature()),
    curve:                      curveVal,
...
class IkaService:
    ...
    def _compute_dkg_locally(self, address: str, curve: int = 0) -> dict:
        ...
        with tempfile.NamedTemporaryFile(suffix=".js", mode="w", delete=False, dir=self.sdk_path) as f:
            f.write(js)
            tmp = f.name
